...
public class TesteRei {
	...
	public void podeAtacar() {
		...
		Assert.assertFalse(reiBranco.podeAtacar(new Posicao(6, 4), new Posicao(
				6, 5), tabuleiro));
		Assert.assertFalse(reiBranco.podeAtacar(new Posicao(7, 3), new Posicao(
				6, 2), tabuleiro));
		Assert.assertFalse(reiBranco.podeAtacar(new Posicao(2, 6), new Posicao(
				3, 6), tabuleiro));

		/** Verifica se o rei pode atacar uma posição com peça aliada */
		Assert.assertFalse(reiBranco.podeAtacar(new Posicao(3, 3), new Posicao(
				2, 2), tabuleiro));
	}
}